...
const allsame = (arr) =>
  arr.every(
    (ele) => ele.innerText !== "" && ele.innerText === arr[0].innerText
  );
...
const checkVictory = (ele, rm) => {
  ...
  Gaincombo.forEach((_c) => {
    ...
    if (arr.indexOf(ele) != -1 && allsame(seq)) {
      victory = true;
      if (rm === 0) {
        alert("U got a chance to remove other player coin");
      }
    }
  });
  return victory;
...
const fun = ($event) => {
  if (
    check(qNumid($event.target), val) &&
    checkVictory(qNumid($event.target), 1) === false
  ) {
    rem(qNumid($event.target));
    come2();
  } else {
    alert("Invalid move -- remove other coin");
  }
};
const fn = ($event) => {
  if (
    check(qNumid($event.target), "") &&
    mv[ind].indexOf(qNumid($event.target)) != -1
  ) {
...
const enable2 = () =>
  grid().forEach((_qe1) => _qe1.addEventListener("click", fn));
const disable2 = () =>
  grid().forEach((_qe1) => _qe1.removeEventListener("click", fn));
const enable3 = () =>
  grid().forEach((_qe1) => _qe1.addEventListener("click", fun));
const disable3 = () =>
  grid().forEach((_qe1) => _qe1.removeEventListener("click", fun));
const enable = () =>
